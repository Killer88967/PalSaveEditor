...
pub fn resolve_scalar_mut<'a>(
    ...
    let property = match first {
        PathSegment::Property { name, index } => save
            .root
            .properties
            .0
            .get_mut(&PropertyKey(*index, name.clone()))
            .ok_or_else(|| format!("property {name}_{index} was not found"))?,
        _ => return Err("a scalar path must begin with a property segment".to_string()),
    };
...
fn apply_property_scalar(
    ...
    match property {
        ...
        _ => return Err("this property type is not editable".to_string()),
    }
...
fn apply_value_scalar(
    ...
    match values {
        ...
        _ => return Err("this collection element type is not editable".to_string()),
    }
