...
mod tests {
    ...
    #[test]
    fn revision_increments_and_dirty_after_success() {
        let mut dirty = false;
        let mut revision = 0;
        assert_eq!(
            run_revisioned_mutation(&mut dirty, &mut revision, 0, || Ok::<_, ApiError>(42))
                .unwrap(),
            42
        );
    ...
    #[test]
    fn failed_mutation_preserves_revision_and_dirty() {
        let mut dirty = false;
        let mut revision = 4;
